...
export function createConwayClient(options: ConwayClientOptions): ConwayClient {
  ...
  const exposePort = async (port: number): Promise<PortInfo> => {
    ...
    return { port: result.port, publicUrl: result.public_url || result.publicUrl || result.url, sandboxId };
  };
  ...
  const createSandbox = async (opts: CreateSandboxOptions): Promise<SandboxInfo> => {
    ...
    return {
      ...
      vcpu: result.vcpu || opts.vcpu || 1,
      memoryMb: result.memory_mb || opts.memoryMb || 512,
      diskGb: result.disk_gb || opts.diskGb || 5,
  ...
  const listSandboxes = async (): Promise<SandboxInfo[]> => {
    ...
    const list = Array.isArray(result) ? result : result.sandboxes || [];
  ...
  const transferCredits = async (
    ...
    for (const p of paths) {
      ...
      if (!resp.ok) {
        ...
        throw new Error(`Conway API error: POST ${p} -> ${lastError}`);
      }
  ...
  const registerDomain = async (domain: string, years: number = 1): Promise<DomainRegistration> => {
  ...
  return {
    ...
    getCreditsBalance, getCreditsPricing, transferCredits,
    searchDomains, registerDomain, listDnsRecords, addDnsRecord, deleteDnsRecord,
    listModels,
  };
}
